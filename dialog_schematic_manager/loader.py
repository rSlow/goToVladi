...
from .buttons import BuildingButton
...
class ButtonConfigLoader(ABC):
    ...
    @abstractmethod
    async def load_button_config(self, button_config_identifier: Hashable) -> BuildingButton: ...

    @abstractmethod
    async def save_button_config(self, button: BuildingButton) -> Hashable: ...
...
class PostgresqlMessageConfig:
    ...
    button_data: Mapped[dict] = mapped_column(JSON)
...
class PostgresqlButtonConfigLoader(ButtonConfigLoader):
    ...
    async def load_button_config(self, button_config_identifier: int) -> BuildingButton:
        async with self._session_maker() as session:
            res = await session.scalars(
                select(self._database)
                .where(self._database.id == button_config_identifier)
            )
            button_config = res.one_or_none().button_data
            button_class: BuildingButton = ApiButton.__button_types__[button_config["type_name"]]
            return button_class.model_validate(button_config)
